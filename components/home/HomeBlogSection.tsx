    ...
    const fetchBlogPosts = async () => {
      try {
        ...
        try {
          ...
          console.error('No featured posts available, trying recent posts', error);
          try {
            const recentResponse = await axios.get(`${API_URL}/blog-posts/recent/`);
            posts = recentResponse.data;
          } catch (error) {
            console.error('No recent posts available, trying all posts', error);
            try {
              const allResponse = await axios.get(`${API_URL}/blog-posts/`);
              posts = allResponse.data;
            } catch (error) {
              console.error('No blog posts could be fetched at all', error);
            }
          }
        }
